...
if st.session_state.premium == False:
    with st.expander(" Sei un UTENTE PREMIUM 👑 ? "):
            ...
            st.dataframe(sezioneAnalisi.style.applymap(lambda x: 'background-color: lightgreen' if x == 1 else '', subset=['Analisi']))
            st.markdown("2️⃣ Sezione Ricerca ", unsafe_allow_html=True)
            sezioneRicerca = pd.read_csv("Ricerca.csv")
            st.dataframe(sezioneRicerca.style.applymap(lambda x: 'background-color: lightgreen' if x == 1 else '', subset=['Ricerca']))
            st.markdown("3️⃣ Sezione Competitor ", unsafe_allow_html=True)
            sezioneCompetitor = pd.read_csv("Competitor.csv")
            st.dataframe(sezioneCompetitor.style.applymap(lambda x: 'background-color: lightgreen' if x == 1 else '', subset=['Competitor']))
            st.markdown("4️⃣ Sezione Domande ", unsafe_allow_html=True)
            sezioneDomande = pd.read_csv("Domande.csv")
            st.dataframe(sezioneDomande.style.applymap(lambda x: 'background-color: lightgreen' if x == 1 else '', subset=['Domande']))
            st.markdown("5️⃣ Sezione Contenuti ", unsafe_allow_html=True)
            sezioneContenuti = pd.read_csv("Testo.csv")
            st.dataframe(sezioneContenuti.style.applymap(lambda x: 'background-color: lightgreen' if x == 1 else '', subset=['Contenuti']))
            st.write("  ")
            st.write("  ")
else:
